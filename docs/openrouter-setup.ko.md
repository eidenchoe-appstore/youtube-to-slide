...
3. 한 장만 정리하려면 **Study Selected**, 전체 노트를 수동으로 만들려면 **Study All Slides**, 전체 슬라이드 Notion export까지 한 번에 하려면 **Note to Notion Page**를 누릅니다.
...
슬라이드 추출 후 **Note to Notion Page**를 누릅니다. 앱은 전체 슬라이드를 확인하고, 누락된 슬라이드 공부노트를 생성한 뒤 Notion-ready ZIP을 만듭니다.
...
Markdown은 Notion import에 안전한 heading 계층을 갖습니다.

```markdown
# 강의 제목

## Slide 1: VLM이 추론한 슬라이드 제목
...
Notion에 가져올 때는 ZIP을 import하면 Markdown과 참조 이미지가 함께 처리됩니다. export는 Notion import 안정성을 위해 HTML 태그, 내부 anchor link, 각주, Mermaid, 복잡한 표를 피합니다.

참고: [Notion enhanced Markdown format](https://developers.notion.com/guides/data-apis/enhanced-markdown).

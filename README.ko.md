...
- HTML과 슬라이드 PNG assets를 함께 묶은 **Note to Notion Page** ZIP 생성
...
2. **Save Key**를 누릅니다.
3. 사용할 모델을 선택합니다.
4. **Study Selected**, **Study All Slides** 또는 채팅 질문을 실행합니다.
5. **Note to Notion Page**를 눌러 HTML 페이지와 슬라이드 이미지 assets가 포함된 ZIP을 만듭니다.
...
| Note to Notion Page ZIP | 공부노트와 슬라이드 이미지를 Notion에 함께 가져갈 때. Notion 스타일 HTML과 `assets/` 폴더 포함 |
...
- v1.2.1에는 GPU/Metal 비교가 구현되어 있지 않습니다. 나중에 acceleration을 붙일 수 있도록 처리 엔진은 분리해두었습니다.

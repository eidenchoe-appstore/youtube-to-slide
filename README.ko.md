...
- 누락된 슬라이드 공부노트를 생성하고 HTML과 PNG assets를 묶는 전체 슬라이드 **Note to Notion Page** ZIP 생성
...
슬라이드 추출이 끝난 뒤 오른쪽 inspector에서 OpenRouter API key를 저장합니다.

1. OpenRouter API key를 붙여 넣습니다.
2. **Save Key**를 누릅니다.
3. 사용할 모델을 선택합니다.
4. 수동으로 공부하려면 **Study Selected**, **Study All Slides** 또는 채팅 질문을 실행합니다.
5. **Note to Notion Page**를 누르면 모든 슬라이드의 누락된 공부노트를 생성한 뒤, HTML 페이지와 슬라이드 이미지 assets가 포함된 ZIP을 만듭니다.

ZIP은 Notion import를 염두에 둔 형식입니다. 내부에는 `Title.html`과 참조되는 PNG 슬라이드가 들어 있는 `assets/` 폴더가 포함됩니다.
...
| Note to Notion Page ZIP | 전체 슬라이드 공부 페이지를 Notion에 가져갈 때. 누락된 슬라이드 노트를 생성한 뒤 Notion 스타일 HTML과 `assets/` 폴더를 함께 패키징 |
...
- v2.0.0에는 GPU/Metal 비교가 구현되어 있지 않습니다. 나중에 acceleration을 붙일 수 있도록 처리 엔진은 분리해두었습니다.

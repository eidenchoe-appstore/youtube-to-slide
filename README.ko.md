...
macOS에서 강의 영상을 슬라이드 이미지, PDF, PowerPoint 덱으로 변환하는 앱입니다.

YouTube to Slide는 YouTube 링크나 로컬 영상 파일에서 정적인 강연 슬라이드를 추출하는 로컬 macOS 앱입니다. 강의 영상을 넣고, 샘플링 간격과 변화율 기준을 조절한 뒤 PNG 폴더, PDF, PPTX로 내보낼 수 있습니다.
...
- 기본 1초 샘플링, 25% 변화율 threshold
- PNG 폴더, PDF, PPTX, timeline JSON export
...
앱은 영상을 일정 간격으로 샘플링하고, 직전 샘플 프레임과 현재 프레임의 픽셀 변화율을 계산합니다. 변화율이 기준 이상이면 새 슬라이드로 저장합니다.
...
| PDF | 읽기, 인쇄, 공유용 |
| PPTX | PowerPoint에서 이미지 기반 deck으로 열 때 |
...
| --- | ---: | ---: |
| 정적인 슬라이드 강의 | 1.0초 | 25% |
| bullet animation이 많은 강의 | 0.5초 | 10-15% |
| 변화가 적은 긴 강의 | 2.0초 | 25-35% |
...
- PPTX 출력은 각 슬라이드 이미지를 한 장씩 넣는 방식입니다.
- 필기, 커서 이동, 애니메이션, 발표자 오버레이가 많은 영상은 threshold 조절이나 수동 정리가 필요할 수 있습니다.
- v1.0.1에는 GPU/Metal 비교가 구현되어 있지 않습니다. 나중에 acceleration을 붙일 수 있도록 처리 엔진은 분리해두었습니다.

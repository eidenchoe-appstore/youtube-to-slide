...
macOS에서 강의 영상을 슬라이드 이미지, PDF, PowerPoint 덱, 공부노트로 변환하는 앱입니다.
...
> 화면 공유 기반 강의 영상에 맞춰 설계했습니다. AI, OCR, CLIP embedding, 클라우드 처리를 사용하지 않습니다.
...
- PNG 이미지 상대 링크가 포함된 Notion-friendly Markdown export
...
앱의 YouTube 입력창에 링크를 붙여 넣고, 미리보기 카드가 뜨면 **Add**를 누른 뒤 **Start**를 누르면 됩니다.
...
슬라이드 추출이 끝난 뒤 오른쪽 inspector의 **AI Study Notes**에서 설정합니다.

1. OpenRouter API key를 붙여 넣습니다.
2. **Save Key**를 누릅니다.
3. 사용할 모델을 선택합니다.
4. **Study Selected**, **Study All Slides** 또는 채팅 질문을 실행합니다.
5. **Export Markdown**을 눌러 Notion에 가져갈 수 있는 Markdown 파일을 만듭니다.
...
| Markdown notes | Notion import용. `![Slide](./slide.png)` 상대 이미지 링크 포함 |
...
처리는 로컬에서 수행됩니다. 로컬 영상은 Mac 밖으로 나가지 않습니다. YouTube 링크는 `yt-dlp`로 로컬에 다운로드한 뒤 처리합니다. 앱은 영상 프레임, 슬라이드 이미지, 결과 파일을 AI 서비스로 전송하지 않습니다.
...
- v1.1.0에는 GPU/Metal 비교가 구현되어 있지 않습니다. 나중에 acceleration을 붙일 수 있도록 처리 엔진은 분리해두었습니다.

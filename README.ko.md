...
- 누락된 슬라이드 공부노트를 생성하고 Markdown과 PNG assets를 묶는 전체 슬라이드 **Note to Notion Page** ZIP 생성
...
AI 공부노트 기능에는 사용자의 OpenRouter API key가 필요합니다. API key는 macOS Keychain에 저장되며, 사용자가 공부노트 생성이나 채팅 버튼을 누를 때만 사용됩니다.

설정 방법은 [YouTube to Slide OpenRouter 설정 가이드](docs/openrouter-setup.ko.md)를 참고하세요. 공식 문서는 [OpenRouter Quickstart](https://openrouter.ai/docs/quickstart)와 [API Authentication](https://openrouter.ai/docs/api/reference/authentication)에 연결해두었습니다.
...
5. **Note to Notion Page**를 누르면 모든 슬라이드의 누락된 공부노트를 생성한 뒤, Markdown 페이지와 슬라이드 이미지 assets가 포함된 ZIP을 만듭니다.

ZIP은 Notion import를 염두에 둔 형식입니다. 내부에는 `Title.md`와 참조되는 PNG 슬라이드가 들어 있는 `assets/` 폴더가 포함됩니다. 기존 HTML export보다 Notion import에서 더 안정적인 경로입니다.
...
| 모델 | 장점 |
| --- | --- |
| `nvidia/nemotron-nano-12b-v2-vl:free` | 기본값. OCR, 문서 이해, 차트, 슬라이드 단위 multimodal 이해에 적합 |
| `google/gemma-4-31b-it:free` | 이미지 이해, 다국어 설명, 긴 컨텍스트 종합, instruction following이 강함 |
| `nvidia/llama-nemotron-rerank-vl-1b-v2:free` | 속도가 중요할 때 가벼운 visual relevance check 용도로 유리 |
...
| Note to Notion Page ZIP | 전체 슬라이드 공부 페이지를 Notion에 가져갈 때. 누락된 슬라이드 노트를 생성한 뒤 Notion-safe Markdown과 `assets/` 폴더를 함께 패키징 |
...
OpenRouter 공부노트/채팅 기능은 사용자가 버튼을 누른 뒤 선택된 슬라이드 PNG와 프롬프트를 OpenRouter로 전송합니다. API key는 Keychain에 저장합니다.
...
- v2.1.0에는 GPU/Metal 비교가 구현되어 있지 않습니다. 나중에 acceleration을 붙일 수 있도록 처리 엔진은 분리해두었습니다.
